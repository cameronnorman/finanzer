...
export const getAll = async (profileId: string) => {
  ...
  const profile = await profileRepository.findOne({ where: { id: profileId }, relations: ["categories"] })
...
export const create = async (profileId: string, categoryParams: any) => {
  ...
  const categoryDetails: CategoryDetails = {name: categoryParams.name, profile}
